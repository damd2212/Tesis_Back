...
consumidores_df = pd.read_csv(pathCSV)
...
def lista_conteo_riesgo_edad(request):
    ...
    if request.method == 'GET':
        tabla_contingencia = pd.crosstab(consumidores_df['per_edad_tipo'], consumidores_df['CatRiesgo'])
...
def lista_conteo_riesgo_situacion_actual(request):
    ...
    if request.method == 'GET':
        ...
        tabla_contingencia = pd.crosstab(consumidores_df['situacion_tipo'], consumidores_df['CatRiesgo'])
...
def lista_conteo_riesgo_per_sexo_tipo(request):
    ...
    if request.method == 'GET':
        ...
        tabla_contingencia = pd.crosstab(consumidores_df['per_sexo_tipo'], consumidores_df['CatRiesgo'])
...
def lista_conteo_nivel_edu_marihuana(request):
    ...
    if request.method == 'GET':
        ...
        tabla_contingencia = pd.crosstab(consumidores_df[filtro1_param], consumidores_df[filtro2_param])
...
def lista_conteo_depto_v2(request):
    ...
    if request.method == 'GET':
        data_depto = consumidores_df[filtro_param].value_counts()
...
def lista_registros(request):
    ...
    if request.method == 'GET':
        consumidoresDict = consumidores_df.to_dict('records')
...
def lista_conteo_depto(request):
    ...
    if request.method == 'GET':
        ...
        data_inicial = consumidores_df['departamento'].value_counts()
        dict_data_inicial = data_inicial.to_dict()       
        
        conteo_data = consumidores_df[consumidores_df['CatRiesgo'] == num_filtro]['departamento'].value_counts()
...
def conteo_riesgo(request):
    ...
    if request.method == 'GET':
        conteo_data_riesgo = consumidores_df['CatRiesgo'].value_counts()
        if filtro_depto != 'todos':
            conteo_data_riesgo = consumidores_df[consumidores_df['departamento'] == filtro_depto]['CatRiesgo'].value_counts()
...
def guardarNuevoRegistro(prediccion, obs_df):
    global consumidores_df
    obs_df['CatRiesgo'] = prediccion
    obs_df = obs_df.reindex(columns=consumidores_df.columns)
    consumidores_df = pd.concat([consumidores_df, obs_df], ignore_index=True)
    consumidores_df.to_csv(pathCSV, index=False)
    print("Nuevo registro guardado de forma correcta")
